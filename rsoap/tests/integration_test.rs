...
fn full_serialize_deserialize_round_trip() {
      ...
      struct WeatherReq { zip: String }

         #[derive(Debug, serde::Deserialize)]
        struct WeatherRsp { temp: Option<f64> }

     let req = WeatherReq { zip: "90210".into() };
...
fn soap_operation_trait_exists() {
      ...
      impl SoapOperation for DummyOp {
         ...
         type Response = ();
           ...
           fn parse_response(
                  &self,
                 _response_xml: &str,
              ) -> Result<Self::Response, rsoap::SoapError>
           where
     ...
     let _op = DummyOp;
       assert_eq!(<DummyOp as SoapOperation>::ACTION, "http://example.com/DummyOp");
...
#[derive(Debug, serde::Serialize)]
struct WeatherReqE2e { zip_code: String }

#[derive(Debug, PartialEq, serde::Deserialize)]
struct WeatherRspE2e { temperature: f64 }
...
async fn e2e_successful_call() {
    ...
    let result: Result<WeatherRspE2e, _> = client.call(&TestOp, &WeatherReqE2e { zip_code: "90210".into() }).await;

     assert!(result.is_ok(), "expected successful call, got error: {:?}", result.err());
...
async fn e2e_soap_fault() {
    ...
    let client = SoapClient::new(mock_server.uri()).unwrap();
    let result: Result<WeatherRspE2e, _> = client.call(&TestOp, &WeatherReqE2e { zip_code: "10001".into() }).await;
...
async fn e2e_http_error() {
    ...
    let client = SoapClient::new(mock_server.uri()).unwrap();
    let result: Result<WeatherRspE2e, _> = client.call(&TestOp, &WeatherReqE2e { zip_code: "0".into() }).await;
...
async fn e2e_request_body_check() {
    let mock_server = wiremock::MockServer::start().await;
     ...
     wiremock::Mock::given(wiremock::matchers::method("POST"))
           .and(wiremock::matchers::header("Content-Type", "text/xml; charset=utf-8"))
            .and(|req: &wiremock::Request| {
                String::from_utf8(req.body.clone()).unwrap_or_default().contains("90210")
             })
           .respond_with(wiremock::ResponseTemplate::new(200).set_body_string(soap_response))
         .mount(&mock_server)
         .await;
     ...
     let result: Result<WeatherRspE2e, _> = client.call(&TestOp, &WeatherReqE2e { zip_code: "90210".into() }).await;
      ...
      assert!(result.is_ok(), "expected successful call with body check, got error: {:?}", result.err());
...
async fn e2e_custom_headers_sent() {
    ...
    let result: Result<WeatherRspE2e, _> = client.call(&TestOp, &WeatherReqE2e { zip_code: "30301".into() }).await;

     assert!(result.is_ok(), "expected successful call with auth header, got error: {:?}", result.err());
}
